...
### **Important: How Vercel Deploys Both Parts**

Vercel uses the `vercel.json` file to understand your project has:
1. **Static frontend** (client folder) - Built once, served from CDN
2. **Serverless backend** (server folder) - Runs as API routes

**No need to choose!** Both are deployed together automatically.
...
- Build the **frontend** from `client/` folder
- Deploy the **backend** from `server/` folder
- Use the `vercel.json` configuration

**Build Settings** (Vercel auto-detects these):
- **Framework Preset**: Other
- **Root Directory**: `.` ← Leave this as root!
...
| Name | Value | Example |
|------|-------|---------|
| `MONGODB_URI` | Your MongoDB Atlas connection string | `mongodb+srv://user:pass@cluster.mongodb.net/health-records` |
| `JWT_SECRET` | Random secret key (32+ characters) | `a7f3b9c2d8e1f4a6b7c8d9e0f1a2b3c4d5e6f7a8b9c0d1e2` |
...
3. You'll get a URL like: `https://health-record-system.vercel.app`
